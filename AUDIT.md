...
| **CSS** | ✅ OK (te optimaliseren) | `belasting.<hash>.min.css` 32,4 KB (content-hash in de naam, zie scripts/hash-css.js), render-blocking `<link rel="stylesheet">` (lokaal) | Acceptabel. Optioneel: kritieke CSS inlinen voor snellere first paint. Geen "print-trick" meer → geen CLS-hack. |
